...
项目服务端已经收敛为 `backend/go` 下的单个 Go 进程：
...
构建产物默认是 `backend/go/bin/ai-marketing`。可以用环境变量指定地址和数据目录：

```bash
DATA_DIR=/var/lib/ai-marketing PORT=7072 ./backend/go/bin/ai-marketing
```

## 运行边界

视频模型、对象存储和外部平台仍然是网络依赖；Go 单文件不代表这些第三方服务被打进
