...
仓库当前基线是 `0.1.0`，所以下一次 Docker 或 Electron 发布构建会生成 `0.1.1`。
...
`pnpm version:sync` 用于把 `VERSION` 同步到所有包清单。Docker 和 Electron 发布脚本会在
构建前自动递增版本，构建失败时恢复旧版本。普通开发和 Vite 构建只读取当前版本，不会递增。
...
使用外部同源脚本是为了兼容页面现有的 `script-src 'self'` CSP。Docker 中的 `version.js` 禁止
缓存，确保部署完成后读取到当前产物版本。
