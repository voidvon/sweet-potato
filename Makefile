...
embed-static:
	...
	cd "$(FRONTEND_ADMIN_DIR)" && pnpm run build
	rm -rf "$(STATIC_WEB_DIR)" "$(STATIC_ADMIN_DIR)"
	mkdir -p "$(STATIC_WEB_DIR)" "$(STATIC_ADMIN_DIR)"
	cp -R "$(FRONTEND_WEB_DIR)/dist/." "$(STATIC_WEB_DIR)/"
	cp -R "$(FRONTEND_ADMIN_DIR)/dist/." "$(STATIC_ADMIN_DIR)/"
