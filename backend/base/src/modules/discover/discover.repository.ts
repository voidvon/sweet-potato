...
type ItemRow = { id: string; category_id: string; source_asset_id: string; title: string; description: string; media_type: DiscoverMediaType; mime_type: string; file_url: string; cover_url: string; original_file_name: string; file_size: number; like_count: number; view_count: number; duration: number; source_created_at: string | null; source_completed_at: string | null; reference_assets: string; aspect_ratio: string; status: DiscoverStatus; sort_order: number; published_at: string | null; created_at: string; updated_at: string }
...
const mapItem = (row: ItemRow): DiscoverItem => ({ id: row.id, categoryId: row.category_id, sourceAssetId: row.source_asset_id, title: row.title, description: row.description, mediaType: row.media_type, mimeType: row.mime_type, fileUrl: row.file_url, coverUrl: row.cover_url || '', originalFileName: row.original_file_name, fileSize: Number(row.file_size || 0), likeCount: Number(row.like_count || 0), viewCount: Number(row.view_count || 0), duration: Number(row.duration || 0), sourceCreatedAt: row.source_created_at, sourceCompletedAt: row.source_completed_at, referenceAssets: parseReferenceAssets(row.reference_assets), aspectRatio: row.aspect_ratio || '1 / 1', status: row.status, sortOrder: row.sort_order, publishedAt: row.published_at, createdAt: row.created_at, updatedAt: row.updated_at })
...
export const discoverRepository = {
  ...
  createItem(input: Omit<DiscoverItem, 'id' | 'createdAt' | 'updatedAt' | 'publishedAt'> & { publishedAt?: string | null }) {
    const now = new Date().toISOString(); const id = randomUUID()
    db.prepare(`INSERT INTO discover_items (id, category_id, source_asset_id, title, description, media_type, mime_type, file_url, cover_url, original_file_name, file_size, like_count, view_count, duration, source_created_at, source_completed_at, reference_assets, aspect_ratio, status, sort_order, published_at, created_at, updated_at) VALUES (@id, @categoryId, @sourceAssetId, @title, @description, @mediaType, @mimeType, @fileUrl, @coverUrl, @originalFileName, @fileSize, @likeCount, @viewCount, @duration, @sourceCreatedAt, @sourceCompletedAt, @referenceAssetsJson, @aspectRatio, @status, @sortOrder, @publishedAt, @createdAt, @updatedAt)`).run({ id, ...input, coverUrl: input.coverUrl || '', referenceAssetsJson: JSON.stringify(input.referenceAssets), createdAt: now, updatedAt: now, publishedAt: input.status === 'published' ? (input.publishedAt || now) : null })
