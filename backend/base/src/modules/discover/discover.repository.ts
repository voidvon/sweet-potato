...
import type { DiscoverCategory, DiscoverItem, DiscoverMediaType, DiscoverReferenceAsset } from './discover.types.js'
...
type ItemRow = { id: string; category_id: string; source_asset_id: string; title: string; description: string; media_type: DiscoverMediaType; mime_type: string; file_url: string; cover_url: string; original_file_name: string; file_size: number; like_count: number; view_count: number; duration: number; source_created_at: string | null; source_completed_at: string | null; reference_assets: string; aspect_ratio: string; published_at: string | null; created_at: string; updated_at: string }
...
const mapItem = (row: ItemRow): DiscoverItem => ({ id: row.id, categoryId: row.category_id, sourceAssetId: row.source_asset_id, title: row.title, description: row.description, mediaType: row.media_type, mimeType: row.mime_type, fileUrl: row.file_url, coverUrl: row.cover_url || '', originalFileName: row.original_file_name, fileSize: Number(row.file_size || 0), likeCount: Number(row.like_count || 0), viewCount: Number(row.view_count || 0), duration: Number(row.duration || 0), sourceCreatedAt: row.source_created_at, sourceCompletedAt: row.source_completed_at, referenceAssets: parseReferenceAssets(row.reference_assets), aspectRatio: row.aspect_ratio || '1 / 1', publishedAt: row.published_at, createdAt: row.created_at, updatedAt: row.updated_at })
...
export const discoverRepository = {
  ...
  listItems(input: { categoryId?: string }) {
    const clauses = ['1 = 1']
    const params: Record<string, string> = {}
    if (input.categoryId) { clauses.push('i.category_id = @categoryId'); params.categoryId = input.categoryId }
    return (db.prepare(`SELECT i.* FROM discover_items i WHERE ${clauses.join(' AND ')} ORDER BY i.published_at DESC, i.updated_at DESC, i.id DESC`).all(params) as ItemRow[]).map(mapItem)
  },
  listPublicItems(input: { page: number; pageSize: number; categoryId?: string; mediaType?: DiscoverMediaType; search?: string }) {
    const clauses = ["EXISTS (SELECT 1 FROM discover_categories c WHERE c.id = i.category_id AND c.status = 'active')"]
    ...
    const items = (db.prepare(`SELECT i.* FROM discover_items i WHERE ${where} ORDER BY i.published_at DESC, i.updated_at DESC, i.id DESC LIMIT @limit OFFSET @offset`).all(params) as ItemRow[]).map(mapItem)
  ...
  createItem(input: Omit<DiscoverItem, 'id' | 'createdAt' | 'updatedAt' | 'publishedAt'> & { publishedAt?: string | null }) {
    const now = new Date().toISOString(); const id = randomUUID()
    db.prepare(`INSERT INTO discover_items (id, category_id, source_asset_id, title, description, media_type, mime_type, file_url, cover_url, original_file_name, file_size, like_count, view_count, duration, source_created_at, source_completed_at, reference_assets, aspect_ratio, published_at, created_at, updated_at) VALUES (@id, @categoryId, @sourceAssetId, @title, @description, @mediaType, @mimeType, @fileUrl, @coverUrl, @originalFileName, @fileSize, @likeCount, @viewCount, @duration, @sourceCreatedAt, @sourceCompletedAt, @referenceAssetsJson, @aspectRatio, @publishedAt, @createdAt, @updatedAt)`).run({ id, ...input, coverUrl: input.coverUrl || '', referenceAssetsJson: JSON.stringify(input.referenceAssets), createdAt: now, updatedAt: now, publishedAt: input.publishedAt || now })
    return this.findItem(id)!
  },
  updateItem(id: string, input: Partial<Pick<DiscoverItem, 'categoryId' | 'title' | 'description'>>) {
    const current = this.findItem(id); if (!current) return null
    const now = new Date().toISOString()
    db.prepare('UPDATE discover_items SET category_id = ?, title = ?, description = ?, updated_at = ? WHERE id = ?').run(input.categoryId ?? current.categoryId, input.title ?? current.title, input.description ?? current.description, now, id)
    return this.findItem(id)
  },
