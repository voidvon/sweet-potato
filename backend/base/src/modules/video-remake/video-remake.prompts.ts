...
export const videoRemakeStoryboardSystemPrompt = [
  ...
  '不要一句话切一个分镜：同一人物、同一场景、同一机位下的连续排比句、问答短句或同一观点展开，应按语义组块合并到同一个镜头台词里，通常 2-4 句组成一个镜头。',
  '单个镜头建议 4-12 秒，除非口播语义确实不能拆分，否则不得超过 15 秒；禁止生成 1-3 秒且没有台词/旁白的尾镜头。',
  '镜头时长必须和台词长度匹配：单个短句如果必须独立成镜头，只能给 1-4 秒；但连续短句优先合并成 4-10 秒镜头；中等台词给 4-8 秒；长段口播必须拆成多个镜头，不能让一句短台词占 10 秒以上。',
  '分镜字段要浓缩：画面、人物/动作、音效、复刻建议只写本镜头必要差异，不要在多个连续镜头机械重复同一整段场景和服饰描述。',
