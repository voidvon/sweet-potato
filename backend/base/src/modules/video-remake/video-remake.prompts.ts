...
export const videoRemakeStoryboardSystemPrompt = [
  '你是爆款复刻的分镜脚本分析专家。你需要在用户确认内容后，生成可执行的分镜脚本。',
  '每个镜头必须包含明确时间段，时间段必须连续、不重叠，从 0 秒开始，时间单位为秒数且必须为整数秒。',
  '台词切分必须保持语义完整；不要把完整词语、短语、上下文依赖的连续短句或未结束语义拆到两个镜头，必须按自然语义边界切分。',
  ...
  '已确认口播里如果存在“口播：”“旁白：”“人物X：”等多个说话主体时前缀必须保留；输出分镜时去掉“时间：0s-3s”等时间标注，只保留主体前缀和台词正文。',
  '长口播不能全部塞进第一个镜头再生成无台词短尾镜头；必须按语义自然断点把长口播拆成多个连续镜头，每个镜头都消费一段原文台词。',
  '不要一句话切一个分镜：同一人物、同一场景、同一机位下的连续排比句、问答短句或同一观点展开，应按语义组块合并到同一个镜头台词里',
  '单个镜头建议 4-14 秒，不得超过 15 秒；禁止生成 1-3 秒且没有台词/旁白的尾镜头。',
  '镜头时长必须和台词长度匹配：单个短句如果必须独立成镜头。',
  '分镜字段要浓缩：不要在多个连续镜头机械重复同一整段场景和服饰描述，如果场景一直不变，尽量在多个镜头里用相同的场景描述，只在人物/动作、音效、复刻建议里写本镜头必要差异。',
...
export function buildVideoRemakeSeedanceAudioBindingLines(voiceSettingData: unknown, referenceAudioIds: string[]) {
  ...
  return audioItems.map((item, index) => {
    ...
    const binding = referenceLabel ? `${roleLabel} 只能绑定 ${referenceLabel}` : roleLabel;
    const hasAudioReference = Boolean(item.assetId || item.groupId || referenceLabel);
    const source = hasAudioReference
      ? '参考声音素材的音色、声线、语速、能量和距离感'
      : '';
    ...
    return [
      ...
      source,
      style,
      hasAudioReference ? '只复用音色和节奏，不复用参考音频原始台词、尾音、杂音或转场声。' : '',
    ]
      .filter(Boolean)
      .join('；');
  });
