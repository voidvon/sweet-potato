...
func planningRemotionComposeInput(session store.ContentPlanningSession, presetID string, motionPlan map[string]any, resolveURL func(string) (string, error)) (map[string]any, error) {
	...
	for index, value := range planScenes {
		...
		scenes = append(scenes, map[string]any{
			...
			"narration": map[string]any{"assetId": stringValue(narration, "assetId"), "url": audioURL, "startMs": numberValue(narration["startMs"], 0), "sourceStartMs": numberValue(narration["sourceStartMs"], 0), "playbackRate": numberValue(narration["playbackRate"], 1), "captions": anySlice(narration["captions"])},
		})
	}
