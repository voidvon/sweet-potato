...
func detailImageGenerationSystemPrompt(contextValue map[string]any) string {
	...
	if aspectRatio == "" || strings.EqualFold(aspectRatio, "auto") {
		ratioInstruction = "界面宽高比为自动：必须优先使用 3:4 或更高的竖向画布与自上而下的单栏排版，以同时适配淘宝移动端和 PC 端；内容较多时优先增加画布高度或拆分章节，不得改成横向画布、多栏密排或通过缩小文字硬塞内容。只有用户明确要求横版，或本章原始技术图必须保持横向结构才能准确呈现时，才可使用横向画布。四周保留安全留白，任何文字、主体、边缘和阴影都不得裁切、溢出、遮挡或贴边。"
	} else {
		ratioInstruction = fmt.Sprintf("界面已选择 %s 宽高比：输出必须严格保持该比例；先通过精简文案、等比缩放主体、拆分章节和调整留白来容纳全部内容，字号不得低于移动端可读标准，四周保留安全留白，禁止裁切、溢出、遮挡或贴边。", aspectRatio)
	}
	...
	return "\n你正在执行【淘宝宝贝详情图生成】，必须遵循以下专属规则：" +
		...
		"\n3.1 折线图保真：只要章节引用的任一素材中包含折线图、曲线图或趋势图，图表内容必须逐细节忠实复现原图。坐标轴方向与名称、刻度范围和间隔、单位、数据点位置与数量、折线走向与形状、峰谷和拐点、线型、图例、标签及各序列对应关系均不得改写、平滑、补点、删减、重算或重新设计；不得为了版式美观生成近似曲线。仅允许在不改变信息对应关系和可辨识度的前提下修改图表颜色，其他细节完全以原素材为准。若无法准确复现，宁可直接保留原图表区域，也不得臆造。" +
		"\n3.2 PDF 页眉页脚清理：PDF 页面顶部和底部的文档页眉、页脚属于必须抛弃的版面噪声，不是可用内容。不得在章节主题、文案、插图方案或成图中引用、复现或改写页码、文档编号、版本号、日期、页眉或页脚 Logo、公司抬头、文件名、路径、版权行、保密标识以及页眉页脚装饰线。截取 PDF 图表、表格或技术图示时必须排除这些区域；正文或图表内部具有实际产品含义的编号、Logo、标签和单位不受此规则影响。" +
