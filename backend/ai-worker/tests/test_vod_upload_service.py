...
class VodUploadServiceTest(unittest.TestCase):
    def test_safe_vod_file_name_uses_unique_local_name_for_chinese_titles(self):
        first = _safe_vod_file_name(
            "纯口播.mp4",
            ".mp4",
            "1781604578732-video-upload-asset.mp4",
        )
        second = _safe_vod_file_name(
            "纯旁白.mp4",
            ".mp4",
            "1781605613123-video-upload-asset.mp4",
        )

        self.assertEqual(first, "video-uploads/1781604578732-video-upload-asset.mp4")
        self.assertEqual(second, "video-uploads/1781605613123-video-upload-asset.mp4")
    ...
    def test_safe_vod_file_name_preserves_readable_ascii_part(self):
        ...
        self.assertEqual(result, "video-uploads/clip-1781605613123-video-upload-asset.mp4")
