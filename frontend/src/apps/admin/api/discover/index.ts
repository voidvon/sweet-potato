...
export type DiscoverCategory = { id: string; name: string; nameEn?: string; slug: string; sortOrder: number; status: 'active' | 'disabled' }
...
export const createDiscoverCategory = (input: { name: string; nameEn?: string; slug?: string }) => request<DiscoverCategory>('/api/admin/discover/categories', { method: 'POST', body: JSON.stringify(input) })
export const updateDiscoverCategory = (id: string, input: Partial<Pick<DiscoverCategory, 'name' | 'nameEn' | 'slug' | 'sortOrder' | 'status'>>) => request<DiscoverCategory>(`/api/admin/discover/categories/${id}`, { method: 'PATCH', body: JSON.stringify(input) })
