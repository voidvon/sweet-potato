...
import { ChevronDown, Copy, Maximize2, Play, Plus, Scan, Trash2, UploadCloud, X } from 'lucide-react'
import type {
  BatchAttempt,
  ...
  if (assetAccept(field) === 'video/*' && field.valueType === 'asset-list') {
    return (
      <MemoBatchVideoReferencePicker
        ...
        onChange={(nextIds) => onUpdate(row.id, field.key, nextIds)}
        onUpload={(file) => onUpload(row, field, [file])}
      />
    )
  }

  if (assetAccept(field) === 'audio/*' && field.valueType === 'asset-list') {
    return (
